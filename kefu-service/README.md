...
客服智能体服务（从 `atguigu_ai` 迁移到 deepagents + LangGraph，Phase 7 收尾产物）。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/invoke` | Agent Protocol 兼容入口（接受 `graph_id` + `input`，返回 `QueryResponse`），供 `deepagents` 联邦网关远程直连 |
| `POST` | `/api/messages` | 保留的 `atguigu_ai` 兼容入口（旧契约），内部复用统一核心逻辑 |
| `GET` | `/health` `/health/live` `/health/ready` | 健康探活 |

## 网关接入

`deepagents/agent/config.py` 中：
...
> 历史说明：`kefu-adapter` 转换层已于 2026-08 移除（无调用方），外部 `atguigu_ai` 退役由运维执行。
