...
async def recall_typed_context(session_id: str, query: str) -> str:
    ...
    try:
        ...
        if pool is None:
            return ""
        memories = await recall_typed(pool, session_id, query)
        if not memories:
            return ""
...
async def remember_episodic(session_id: str, query: str, answer: str) -> None:
