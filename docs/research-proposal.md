...
- **单进程多节点而非微服务**：子项目拆 HTTP 适配层（wenda-adapter，kefu-adapter 已于 2026-08 移除、迁移至 kefu-service 直连）已证明引入 SSE 聚合 bug、会话键错误、探活自递归等连环问题；LangGraph 节点边界隔离，保留未来按节点拆服务能力。
