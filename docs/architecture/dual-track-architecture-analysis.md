...
> 调研日期：2026-08-16
> 关联：架构边界 `docs/architecture-boundary-app-vs-agent-federation.md`、收敛规划 `docs/plan-e-dual-track-convergence.md`
...
- **轨道 B `agent_federation/`**：`agent_federation.create_deep_agent(subagents=[...])` 联邦网关（`agent_federation/agent/main_agent.py:128`），其 checkpointer 仍是 LangGraph 的 `InMemorySaver`（`main_agent.py:37`）——即 **DeepAgents 包是 LangGraph 之上的封装**，而非替代。
...
- `agent_federation`：`agent/intent/classifier.py` + `agent/intent/llm_judge.py` + `agent/rewrite/rewrite_node.py`（L1/L2 意图 + 改写），以及 `database_query_agent`/`network_search_agent`/`knowledge_base_agent` 子 Agent。

**影响**：路由/改写逻辑演化需双份维护；两套意图分类策略可能给出不同结论，行为不一致且难定位。

**趋势**：收敛规划 E-1/E-2 已部分收口（契约断言、SQL 守卫委托内核），但**意图分类与 Query 改写仍是双份**，未对齐到单一真相源。

**建议**：以 `app` 的 `decide_route` + `agent_core` 为路由真相源，agent_federation 侧复用或经 kernel 桥接；或明确「agent_federation 不自带意图分类、委派前由网关层统一」。
...
- `agent_federation`：`agent_federation/agent/config.py` 用 dataclass + dotenv，外加 `prompt/prompts.yml` YAML；能力开关（GUARD/PLANNER/REFLEXION/INTENT/CACHE/AGENT_MODE）散落 env（`main_agent.py:73-214`）。

**影响**：新人需同时理解两套配置范式与两套开关语义；env 变量命名空间可能碰撞，排查成本翻倍。

**建议**：长期将 agent_federation 配置也收敛到 pydantic-settings 或复用 `app` 的 `Settings`；短期至少在 README 用一张表枚举 agent_federation 全部 env 开关与默认值。
...
- `agent_federation` 的 middleware 全开关驱动（`main_agent.py:63-114`）虽便于实验，但也意味着能力是否生效依赖运行时 env，文档与代码易漂移。
...
**现象**：社区或团队可能提出"抽 `app` 节点为 `agent_federation` subagent"或反向图重写以"统一架构"。

**影响**：会同时破坏 `app` 的合规外壳（admission/coordinator/revert/PG checkpoint/sqlglot 双保险/SSE）与 `agent_federation` 的联邦远程治理（429/探活）——收敛规划 §3 已列为硬约束。
...
*生成依据：`app/agent/graph.py`、`agent_federation/agent/main_agent.py`、`docs/architecture-boundary-app-vs-agent-federation.md`、`docs/plan-e-dual-track-convergence.md` 双轨调研；问题编号 AR-1~AR-6，技术债编号 TB-9~TB-13。*
