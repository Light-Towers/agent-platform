...
def milvus_ready() -> bool:
    """
    Milvus 真实连通性探测（/health/ready 探针用）。

    背景：get_milvus_client 为单例缓存，Milvus 停服后缓存对象仍在，仅判
    client is not None 会假阳性。此处对缓存客户端执行轻量操作（list_collections，
    带 3s 超时）验证真实可达；失败则重置单例缓存，下次调用可重建连接。
    """
    global _milvus_client
