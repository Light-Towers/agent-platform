# -*- coding: utf-8 -*-
"""语义长期记忆门面（agent_federation 薄封装 + ADR-0004 阶段2 类型化增强）。
...
（PR #9 / ADR-0004），使 agent_federation 长期记忆具备类型增强，与 app 共用单一真相源。

驱动策略（ADR-0004 约束 / ADR-0003）：
- typed API 以 ``pool`` 为**必需参数**，接收调用方传入的宿主 psycopg 池
  （``%s`` 风格 + ``pool.connection()``），agent_federation 不自建池，避免双池
...
- 不在 main_agent 强制接线：当前 run_deep_agent 无明确记忆落库/召回点，按
  ADR-0004 阶段2 仅暴露 API，避免破坏现有 agent_federation 行为。
"""
...
def embed_memory(text: str) -> list[float]:
    ...
    与 agent_federation 的语义缓存/意图层同手法（``agent_core.memory.embedder.get_embedder``），
