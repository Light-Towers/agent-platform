...
> M1 工程闭环：只补 CI / 版本治理 / 测试分层三项工程能力，不涉及 `knowledge_service/` 业务代码改造。
> M2 检索评测体系 + 索引生命周期版本化：开始触碰 `knowledge_service/` 源码（配置驱动集合命名、chunk 元数据、registry），
> 并落地评测管线（golden dataset / 指标纯函数 / run_eval CLI / 单测）。
> M3 配置外置与实验管理：检索/重排参数外置到 `knowledge_service/conf/*.yaml`，消除节点内硬编码；
...
- **验收项 ⑦ OTel 真实导出跑通（2026-08-06 补验）**：`tracing.py` 修复三处使 OTLP 导出真正可用——
  ...
  + `ZHANGUI_TRACE_ENABLED=true`，`POST /query` 即在 Jaeger(`localhost:16686`) 的 `knowledge-service`
...
- **GitHub Actions CI 工作流**（`.github/workflows/zhiku-ci.yml`，位于 **monorepo 仓库根**；本仓库为
  ...
  `pull_request` 触发（均带 `paths: ['knowledge-service/**']` 过滤，仅子项目变更时触发）。
  ...
  **不推送 Registry**、不引入 `docker/build-push-action`。所有 `run` 步骤通过
  `defaults.run.working-directory: knowledge-service` 定位到子项目。
- **Nightly 工作流**（`.github/workflows/zhiku-nightly.yml`，同样位于 monorepo 仓库根）：UTC 18:00
  ...
  评测产物（artifact `path` 相对 workspace 根，为 `knowledge-service/eval/runs/`）。
...
- **配置接入**：`.env.example` 增 `OTEL_EXPORTER_OTLP_ENDPOINT`（默认空 → no-op）、
  `ZHANGUI_SERVICE_NAME`（默认 knowledge-service）、`ZHANGUI_TRACE_ENABLED`（默认 false，
