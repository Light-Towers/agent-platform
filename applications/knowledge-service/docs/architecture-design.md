# 掌柜智库（knowledge-service）生产级重构 — 架构设计与任务分解
...
| `knowledge_service/__init__.py`、`knowledge_service/core/__init__.py`、`knowledge_service/import_process/__init__.py`、`knowledge_service/import_process/agent/nodes/__init__.py`、`knowledge_service/query_process/api/__init__.py`、`knowledge_service/query_process/agent/nodes/__init__.py`、`knowledge_service/api/__init__.py` | **缺失**（需补，否则 `packages.find` 会漏包） |
| 散落的 `load_dotenv()` | **17 处**（见 §4 清单），需收敛到 `knowledge_service/core/config.py` 单点 |
...
│   │   └── load_prompt.py    # 【迁】不变
│   ├── conf/                 # 【改】7 个文件全部去除 load_dotenv，改 from knowledge_service.core.config import settings
│   │   ├── embedding_config.py
│   │   ├── lm_config.py
│   │   ├── milvus_config.py
│   │   ├── minio_config.py
...
### 2.1 设计原则
- **单点读取**：新建 `knowledge_service/core/config.py`，在模块导入时 **唯一一次** `load_dotenv()`，用 `dataclass` 定义 `Settings`，全部 `os.getenv(...)` 带默认值；导出模块级单例 `settings = Settings()`。
...
- **确定性 PROJECT_ROOT**：`config.py` 中 `PROJECT_ROOT = Path(__file__).resolve().parents[2]`（knowledge_service/core → 上两级 = 仓库根），并允许 `PROJECT_ROOT` 环境变量覆盖。Docker 下无 `.env` 文件也能正确定位，解决原 `path_util.get_project_root()` 依赖 `.env` 存在的隐患。
...
| Mongo | `mongo_db_name` | `MONGO_DB_NAME` | `knowledge_service` | 库名 |
...
```python
# knowledge_service/core/config.py
...
各 `conf/*.py` 改为：删除 `from dotenv import load_dotenv` 与 `load_dotenv()`，删除本地 `os.getenv(...)`；改为 `from knowledge_service.core.config import settings`，并 `milvus_config = MilvusConfig(milvus_url=settings.milvus_url, ...)`。
...
def run():
    import uvicorn
    uvicorn.run("knowledge_service.main:app", host=settings.app_host, port=settings.app_port)
...
### 3.3 router 改造要点
- `file_import_service.py` → `knowledge_service/api/import_router.py`：`app = FastAPI(...)` 改为 `router = APIRouter()`；删除顶部 `uvicorn` 导入与底部 `if __name__ == "__main__"`；`PROJECT_ROOT` 改从 `settings.project_root` 取；`os.getenv("MINIO_PDF_DIR", "pdf_files")` / `os.getenv("MINIO_BUCKET_NAME", "kb-import-bucket")` 改从 `settings.minio_pdf_dir` / `settings.minio_bucket_name`。
- `query_service.py` → `knowledge_service/api/query_router.py`：同上改为 `APIRouter()`；保留 `QueryRequest` 模型与全部处理函数；`from knowledge_service.utils.task_utils import *` 等保持；删除 `__main__` 与硬编码 8001。
...
| 文件 | 动作 | 依据 |
|---|---|---|
| `knowledge_service/clients/mongo_history_utils_new.py` | **删除** | 全仓 grep `mongo_history_utils_new` 零引用 |

### 4.2 去除散落 `load_dotenv()`（共 17 处 → 收敛为 1 处）
以下文件删除 `from dotenv import load_dotenv` 与 `load_dotenv()` 调用，改为 `from knowledge_service.core.config import settings`：
- `knowledge_service/conf/{embedding,lm,milvus,minio,mineru,reranker,bailian_mcp}_config.py`（7）
- `knowledge_service/lm/lm_utils.py`
- `knowledge_service/core/logger.py`（同时 `PROJECT_ROOT` 改从 `settings.project_root`）
- `knowledge_service/clients/mongo_history_utils.py`
- `knowledge_service/utils/path_util.py`（同时 `PROJECT_ROOT` 改从 `settings.project_root`，保留 `get_path_dir` 助手）
- `knowledge_service/import_process/agent/main_graph.py`
- `knowledge_service/query_process/agent/main_graph.py`
- `knowledge_service/query_process/agent/nodes/node_item_name_confirm.py`
- `knowledge_service/import_process/agent/nodes/node_import_milvus.py`
- `knowledge_service/query_process/agent/nodes/node_search_embedding.py`
- `knowledge_service/query_process/agent/nodes/node_search_embedding_hyde.py`

**验收**：`grep -rn "load_dotenv" knowledge_service/` 仅 `knowledge_service/core/config.py` 命中。
...
### 4.6 其他隐患（低优先级）
- `knowledge_service/lm/lm_utils.py` 错误提示写 `OPENAI_API_BASE`，实际变量为 `OPENAI_BASE_URL` → 文案修正。
...
### T1 — 配置集中化（P0）
- **文件**：`knowledge_service/core/config.py`（新）；`knowledge_service/conf/*.py`（×7 改）；`knowledge_service/lm/lm_utils.py`、`knowledge_service/core/logger.py`、`knowledge_service/clients/mongo_history_utils.py`、`knowledge_service/utils/path_util.py`（改）
...
- **依赖**：无
- **验收**：`python -c "from knowledge_service.core.config import settings"` 成功；`grep -rn "load_dotenv" knowledge_service/ | grep -v "knowledge_service/core/config.py"` 为空；`settings.milvus_url` 等取值符合默认。

### T2 — 删除死代码与散落 load_dotenv（P0）
- **文件**：删除 `knowledge_service/clients/mongo_history_utils_new.py`；改 `knowledge_service/import_process/agent/main_graph.py`、`knowledge_service/query_process/agent/main_graph.py`、`knowledge_service/query_process/agent/nodes/node_item_name_confirm.py`、`knowledge_service/import_process/agent/nodes/node_import_milvus.py`、`knowledge_service/query_process/agent/nodes/node_search_embedding.py`、`knowledge_service/query_process/agent/nodes/node_search_embedding_hyde.py`
- **做什么**：删除上述 6 个文件中的 `load_dotenv` 调用；删除死代码文件。
- **依赖**：无（可与 T1 并行）
- **验收**：`grep -rn "load_dotenv" knowledge_service/` 仅 `config.py`；`grep -rn "mongo_history_utils_new" .` 无结果；`python -c "import knowledge_service.import_process.agent.main_graph, app.query_process.agent.main_graph"` 成功。

### T3 — 合并 import 服务为 router（P0）
- **文件**：`knowledge_service/api/import_router.py`（新）；`knowledge_service/import_process/api/__init__.py`（补）；删除/退役 `knowledge_service/import_process/api/file_import_service.py`
...
- **依赖**：T1
- **验收**：`python -c "from knowledge_service.api.import_router import router"` 成功；router 路由路径与源一致；无 `8000` 硬编码。

### T4 — 合并 query 服务为 router（P0）
- **文件**：`knowledge_service/api/query_router.py`（新）；`knowledge_service/query_process/api/__init__.py`（补）；删除/退役 `knowledge_service/query_process/api/query_service.py`
- **做什么**：改写为 `APIRouter`，保留 `/chat.html`、`/health`、`/query`、`/stream/{session_id}`、`/history/{session_id}`(GET/DELETE)；删除 `__main__` 与 `8001` 硬编码。
- **依赖**：T1
- **验收**：`python -c "from knowledge_service.api.query_router import router"` 成功；端点路径一致。

### T5 — 单一入口 main.py + 补齐 __init__.py（P0）
- **文件**：`knowledge_service/main.py`（新）；`knowledge_service/api/__init__.py`、`knowledge_service/__init__.py`、`knowledge_service/core/__init__.py`、`knowledge_service/import_process/__init__.py`、`knowledge_service/import_process/agent/__init__.py`、`knowledge_service/import_process/agent/nodes/__init__.py`、`knowledge_service/query_process/api/__init__.py`、`knowledge_service/query_process/agent/nodes/__init__.py`（补）
- **做什么**：按 §3.2 建 `create_app()` + `run()`；`include_router` 两个 router；补齐所有缺失 `__init__.py`。
- **依赖**：T1, T3, T4
- **验收**：`python -c "from knowledge_service.main import app"` 成功；`uvicorn knowledge_service.main:app --port 8000` 启动后 `GET /health` 返回 `{"ok":true}`；`GET /openapi.json` 含全部 9 个端点。

### T6 — 前端 API_BASE 同源化（P1）
- **文件**：`knowledge_service/import_process/page/import.html`、`knowledge_service/query_process/page/chat.html`
...
- **做什么**：`pyproject.toml` 改 `name="knowledge-service"`、补 `description`/`license`、`[tool.setuptools.packages.find]` 含 `knowledge_service`、加 `[project.scripts] knowledge-service = "knowledge_service.main:run"`；生成 `requirements.txt`（等价依赖列表，可由 `uv export -o requirements.txt` 得到）。
- **依赖**：T1, T5
- **验收**：`uv pip install -r requirements.txt` 或 `pip install -e .` 成功；命令行 `knowledge-service` 可启动服务（等效 `uvicorn knowledge_service.main:app`）。

### T8 — 容器化（P1）
- **文件**：`Dockerfile`、`docker-compose.yml`、`.dockerignore`（新）
- **做什么**：`Dockerfile` 基于 `python:3.11-slim`，安装系统依赖（build-essential 等，适配 torch/magic-pdf），`COPY` 依赖与 `knowledge_service`，`EXPOSE 8000`，`CMD` 跑 `knowledge-service`（或 `uvicorn knowledge_service.main:app`）；`docker-compose.yml` 编排 `milvus-standalone`、`mongo`、`minio`、`neo4j` 与 `web`（本服务），端口/账号对齐 `.env.example`；`.dockerignore` 排除 `.env`、`output/`、`logs/`、`models/`、`__pycache__`。
- **依赖**：T7
- **验收**：`docker compose up --build` 拉起 5 服务；依赖就绪后服务日志无连接错误；`/health` 200。

### T9 — 文档与资源迁移（P2）
- **文件**：`README.md`（新）；迁移 `prompts/`、`test/`、`knowledge_service/tool/`（原样）
- **做什么**：写 `README.md`（架构 Mermaid 图、技术栈、uv/pip 快速开始、环境变量表、docker 用法、API 速查、目录结构、测试、故障排查）；复制 `prompts/`（6 个 .prompt）、`test/`（标注需完整依赖）、`knowledge_service/tool/`（下载脚本保留）。
- **依赖**：T1–T8
- **验收**：README 中命令可复现；`prompts/`、`test/`、`knowledge_service/tool/` 就位；`doc/`、`output/`、`logs/`、`models/` 不在仓库。
