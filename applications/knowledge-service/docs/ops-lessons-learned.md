...
> 本文记录在 `knowledge-service`（掌柜智库）生产化改造与环境验收过程中踩过的坑、
...
- 实测镜像 `knowledge-service-web` 体积 **~6.19GB**，含 `nvidia-cu13` 全套依赖
...
格式化瑕疵，不影响功能），但 Jaeger 里 `knowledge-service` 服务名**始终 0 trace**。
...
- 查 `service=knowledge-service` 自然 0 条——**不是没导出，是归错服务名了**。
...
# 4) 查 Jaeger
curl 'http://localhost:16686/api/traces?service=knowledge-service'
#    应出现 trace，每请求含 request.total + retrieval.rewrite 等 span
```

**实测结果（2026-08-06）**：`knowledge-service` 服务名 count=3，每请求 2 span，无 export error。
...
| 查看容器内 env | `docker exec knowledge-service-web printenv <VAR>` |
