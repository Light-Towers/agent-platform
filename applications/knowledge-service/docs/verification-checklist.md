...
| ④ | `.env` 设 `KNOWLEDGE_API_KEY=secret` 后重启 web | 探针（/health、/health/live、/health/ready）无 key 仍 200（M6 豁免）；`POST /query` 无 key → 401、错 key → 401、`X-API-Key: secret` → 200、`Authorization: Bearer secret` → 200 | **部分通过（2026-08-06）**：当前**未设 `KNOWLEDGE_API_KEY`**，鉴权中间件禁用，所有请求（含探针、/query、/retrieve）均放行（已验：无 key / 错 key / Bearer 全 200）。**已通过（2026-08-06 补验）**：`.env` 设 `KNOWLEDGE_API_KEY`（自生成密钥）并重建 web 后实测：`POST /query` 无 key→401 `UNAUTHORIZED`、错 key→401、`X-API-Key: <key>`→200、Bearer→200；`/health*` 与 `/import.html` 免鉴权仍 200。鉴权中间件（X-API-Key / Authorization: Bearer，secrets.compare_digest 防时序）真实生效。 |
...
| ⑦ | OTel：`docker compose --profile core --profile obs up -d --build`；`.env` 设 `OTEL_EXPORTER_OTLP_ENDPOINT=http://jaeger:4318/v1/traces` 与 `KNOWLEDGE_TRACE_ENABLED=true`；发起一次 `/query` | Jaeger UI 可见 `knowledge-service` 服务；trace 含 `request.total` 根 span + 子 span；span 带 config_hash/collection/request_id | **已通过（2026-08-06 补验）**：重建 web + 起 jaeger(obs)，`.env` 设 `KNOWLEDGE_TRACE_ENABLED=true` + `OTEL_EXPORTER_OTLP_ENDPOINT=http://jaeger:4318/v1/traces`，`POST /query` 后 Jaeger 出现 `knowledge-service` 服务名、count=3、每请求 2 span（request.total + retrieval.rewrite），无 export error/404。修复：tracing.py 改 HTTP 优先 exporter（grpc 握手 failed EOF）、TracerProvider 补 `resource.service.name`（否则归 unknown_service）、endpoint 带 `/v1/traces`（HTTP exporter 不自动追加）。 |
