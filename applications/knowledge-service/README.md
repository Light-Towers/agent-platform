...
flowchart TB
    ...
    subgraph API["单一 FastAPI 应用 (knowledge_service.main:app)"]
...
```bash
knowledge-service            # 等价于 uvicorn knowledge_service.main:app --host 0.0.0.0 --port 8000
```

方式二：直接用 uvicorn

```bash
uvicorn knowledge_service.main:app --host 0.0.0.0 --port 8000
```
...
| Mongo | `MONGO_URL` / `MONGO_DB_NAME` | `mongodb://localhost:27017` / `knowledge-service` | 会话历史库 |
...
- **`.env` 未生效**：`.env` 由 `knowledge_service.core.config` 在导入时**仅加载一次**，请确保在启动进程前已存在；`PROJECT_ROOT` 等也可通过环境变量覆盖。
