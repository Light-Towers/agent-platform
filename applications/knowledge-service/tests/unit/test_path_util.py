# -*- coding: utf-8 -*-
"""
test_path_util.py —— 验证 knowledge_service/utils/path_util.py 的路径工具。

【不依赖重型依赖，可在纯 pytest+numpy+python-dotenv 环境运行。】
path_util 仅依赖 stdlib（pathlib / os）与 knowledge_service.core.config，
而 knowledge_service.core.config 亦为纯逻辑模块，不会拉起重型依赖。
...
from knowledge_service.utils.path_util import get_path_dir, get_project_root
...
def test_get_path_dir_zero_points_to_utils():
    # path_util.py 位于 knowledge_service/utils，parents[0] == knowledge_service/utils
    d = get_path_dir(0)
    assert d.name == "utils"
...
def test_get_path_dir_one_points_to_app():
    d = get_path_dir(1)
    assert d.name == "knowledge_service"
...
def test_get_project_root_default_is_repo_root():
    ...
    try:
        root = get_project_root()
        assert (root / "knowledge_service").is_dir()
    finally:
...
def test_get_project_root_env_override(monkeypatch):
    ...
    from knowledge_service.core.config import PROJECT_ROOT as GLOBAL_ROOT
