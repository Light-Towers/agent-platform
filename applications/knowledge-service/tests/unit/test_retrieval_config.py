...
验证：
1. `knowledge_service/conf/retrieval.yaml` / `knowledge_service/conf/rerank.yaml` 可加载，默认值与改造前硬编码一致
...
【不依赖重型依赖】：本文件只 import knowledge_service.conf.retrieval_config / rerank_config /
...
from knowledge_service.conf import rerank_config, retrieval_config
from knowledge_service.conf.yaml_config_utils import CfgDict, load_yaml_config

# 配置目录基于本包绝对定位（不依赖 pytest 运行 cwd），否则从仓库根运行
# pytest 时相对路径 knowledge_service/conf/retrieval.yaml 会解析到仓库根而非 zhanggui 子包。
_CONF_DIR = Path(__file__).resolve().parents[2] / "knowledge_service" / "conf"
