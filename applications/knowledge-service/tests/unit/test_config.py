# -*- coding: utf-8 -*-
"""
test_config.py —— 验证 knowledge_service/core/config.py 的配置单例与默认值。

【不依赖重型依赖，可在纯 pytest+numpy+python-dotenv 环境运行。】
本测试只 import knowledge_service.core.config（仅依赖 os / dataclasses / pathlib / dotenv），
...
from knowledge_service.core.config import PROJECT_ROOT, Settings, _as_bool, _as_int, settings
...
def test_project_root_points_to_repo_root():
    # PROJECT_ROOT 应解析为仓库根目录，且其中包含 knowledge_service/ 包
    assert PROJECT_ROOT.is_dir()
    assert (PROJECT_ROOT / "knowledge_service").is_dir()
    assert (PROJECT_ROOT / "knowledge_service" / "core" / "config.py").is_file()


def test_settings_is_dataclass_singleton():
    # import 多次返回的 settings 应为同一对象（模块级单例）
    from knowledge_service.core.config import settings as settings2
