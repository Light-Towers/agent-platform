...
验证方式：mock knowledge_service.lm.siliconflow_client._post_json（不真调网络）。
验证重点：
...
from knowledge_service.conf.reranker_config import reranker_config
from knowledge_service.lm import reranker_utils
...
def test_compute_score_order_and_values(monkeypatch):
    ...
    monkeypatch.setattr("knowledge_service.lm.siliconflow_client._post_json", fake_post_json)
