...
覆盖：
1. **fan-out 超时隔离**（`knowledge_service/query_process/agent/fanout.guarded_call`）：
   - 单路超时 → 返回 {}（空状态更新），整体不抛异常；
   - 单路异常 → 返回 {}，不拖垮整体；
   - 正常路径透传节点返回的 dict；非 dict 结果（如 None）归一为 {}；
   - `wrap_channel_node` 从 retrieval.yaml 读 enabled/timeout_s（可注入假配置验证）。
2. **reranker 并发闸门**（`knowledge_service/utils/rerank_concurrency`）：
...
from knowledge_service.conf.yaml_config_utils import CfgDict
from knowledge_service.query_process.agent import fanout
from knowledge_service.utils.rerank_concurrency import call_under_semaphore, make_rerank_semaphore
from knowledge_service.utils.security_guard_utils import (
