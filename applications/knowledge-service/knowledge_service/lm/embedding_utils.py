...
def _get_api_embedding_client():
    ...
    if _api_embedding_client is None:
        ...
        from knowledge_service.lm.siliconflow_client import SiliconFlowEmbeddingClient
...
def _generate_embeddings_api(texts):
    ...
    → 业务节点零改动。
    """
    from knowledge_service.lm.sparse_vectorizer import build_sparse_vectors
...
def generate_embeddings(texts):
    ...
    稀疏由 knowledge_service/lm/sparse_vectorizer.py 本地生成（B 路线）；返回结构与 local 完全一致。
