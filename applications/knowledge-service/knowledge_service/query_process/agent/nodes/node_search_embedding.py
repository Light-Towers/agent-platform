...
from knowledge_service.clients.milvus_utils import create_hybrid_search_requests, get_milvus_client, hybrid_search
...
def _embedding_span_attrs(*args, result=None, **kwargs):
    """retrieval.embedding span 动态属性（M4，方案 §8.2 表：dense_w / sparse_w / hits / timeout）。"""
    hits = len((result or {}).get("embedding_chunks") or []) if isinstance(result, dict) else 0
    return {
        "dense_w": retrieval_cfg.hybrid.dense_weight,
        "sparse_w": retrieval_cfg.hybrid.sparse_weight,
        "hits": hits,
        "timeout_s": retrieval_cfg.channels.embedding.timeout_s,
    }


@traced_span("retrieval.embedding", attributes_fn=_embedding_span_attrs)
def node_search_embedding(state):
    ...
    # 与导入侧（node_import_milvus）统一从 milvus_config.chunks_collection 读取，
    # 均为版本化命名 {prefix}_{schema_version}_{embedding_model}，杜绝两侧写死不一致。
    ...
    # M3（方案 §7.2）：稠密/稀疏评分权重从 knowledge_service/conf/retrieval.yaml 读取（hybrid.dense_weight/sparse_weight）
    res = hybrid_search(
        client=client,
        collection_name=collection_name,  # 检索的目标集合名（文本片段向量集合）
        reqs=reqs,  # 构造好的混合搜索请求对象（稠密+稀疏）
        ranker_weights=(
            retrieval_cfg.hybrid.dense_weight,
