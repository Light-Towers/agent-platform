...
from knowledge_service.query_process.agent.prompt_budget import format_history
from knowledge_service.query_process.agent.state import QueryGraphState
from knowledge_service.utils.sse_utils import SSEEvent, push_to_session
...
def step_2_construct_prompt(state: QueryGraphState) -> str:
    ...
    #    修复（代码审查）：长度累加由「累积拼接后的总长」（O(n²) 高估）改为
    #    「每轮新增文本增量」（O(n)）；history_str 最终内容不变，仅截断时机更准确。
    # ---------------------------------------------------------
    history_str, used = format_history(history, used, MAX_CONTEXT_CHARS)
    ...
    # 5. 组装 Prompt
    prompt = load_prompt(
        "answer_out", context=context_str, history=history_str, item_names=item_names_str, question=question
    )
...
def node_answer_output(state: QueryGraphState) -> QueryGraphState:
    ...
    5 做最后一次push操作（主要是为了触发前端图片渲染)
       {
          ...
          "status": "completed",
          "image_urls": [
              "http://local-server/images/example_view.jpg",
              "http://local-server/images/example_detail.jpg"
          ]
        }
    """
    ...
    if not answer_exists:
        ...
        # M4：llm.generate span（LLM 生成节点，属性：模型名 + 答案长度）
        with start_span("llm.generate", attrs={"model": settings.llm_default_model or "default"}) as gen_span:
            step_3_generate_response(state, prompt)
            gen_span.set_attribute("answer_len", len(state.get("answer") or ""))
