...
from knowledge_service.lm.embedding_utils import *
from knowledge_service.lm.lm_utils import *
...
def _hyde_span_attrs(*args, result=None, **kwargs):
    """retrieval.hyde span 动态属性（M4，方案 §8.2 表：enable_hyde / hyde_doc_len / hits）。"""
    result = result or {}
    hyde_doc = result.get("hyde_doc") or "" if isinstance(result, dict) else ""
    hits = len(result.get("hyde_embedding_chunks") or []) if isinstance(result, dict) else 0
    return {
        "enable_hyde": bool(retrieval_cfg.channels.hyde.enabled),
        "hyde_doc_len": len(hyde_doc),
        "hits": hits,
        "timeout_s": retrieval_cfg.channels.hyde.timeout_s,
    }
...
def step_2_search_embedding_hyde(
    ...
    :param ranker_weights: 混合检索权重 (Dense, Sparse)；None 时读 knowledge_service/conf/retrieval.yaml
    ...
    # 3. 准备 Milvus 检索
    # 与导入侧统一从 milvus_config.chunks_collection 读取版本化集合名，杜绝两侧写死不一致
    collection_name = milvus_config.chunks_collection
    if not collection_name:
        logger.error("Step 2 Error: chunks_collection 未配置")
        return []
...
@traced_span("retrieval.hyde", attributes_fn=_hyde_span_attrs)
