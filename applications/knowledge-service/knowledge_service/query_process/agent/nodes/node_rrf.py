...
def _rrf_span_attrs(*args, result=None, **kwargs):
    """ranking.rrf span 动态属性（M4，方案 §8.2 表：k / weights / in_n / out_n）。"""
    state = args[0] if args and isinstance(args[0], dict) else {}
    in_n = len(_as_entity_list(state.get("embedding_chunks"))) + len(
        _as_entity_list(state.get("hyde_embedding_chunks"))
    )
    return {
        "k": retrieval_cfg.rrf.k,
        "weights": dict(retrieval_cfg.rrf.weights),
        "in_n": in_n,
        "out_n": len((result or {}).get("rrf_chunks") or []),
    }
...
@traced_span("ranking.rrf", attributes_fn=_rrf_span_attrs)
def node_rrf(state):
    """
    ...
    # 第二步：为不同来源设置权重
    # M3（方案 §7.2）：权重 / k / max_results 从 knowledge_service/conf/retrieval.yaml 读取（具名通道，
    # 可扩展到 4 路 embedding/hyde/kg/web）；未配置的通道兜底 1.0，保证新增路不破坏既有融合。
    ...
    for channel, chunks in channel_chunks.items():
        weight = float(rrf_cfg.weights.get(channel, 1.0))
        source_weights.append((chunks, weight))
        if chunks:
            active_channels.append(channel)
    logger.info(
        f"RRF 配置: k={rrf_cfg.k}, max_results={rrf_cfg.max_results}, "
        f"active_channels={active_channels}, "
        f"weights={ {ch: float(rrf_cfg.weights.get(ch, 1.0)) for ch in active_channels} }"
    )

    # 第三步：应用带权重的RRF计算最终得分
    # k / max_results 读取配置（默认与原硬编码 k=60 / max_results=10 一致）
    rrf_res = reciprocal_rank_fusion(source_weights, k=rrf_cfg.k, max_results=rrf_cfg.max_results)
