...
from knowledge_service.clients.mongo_history_utils import (
    get_recent_messages,
    save_chat_message,
...
def step_4_vectorize_and_query(
    ...
    # M3：与检索侧统一从 milvus_config 读取商品名集合（杜绝两侧写死不一致）
...
def step_5_align_item_names(query_results: List[Dict]) -> Dict:
    ...
    result = {"confirmed_item_names": list(set(confirmed_item_names)), "options": list(set(options))}
    logger.info(f"Step 5: 对齐结果: {result}")
    return result


def step_6_check_confirmation(
    state: Dict, align_result: Dict, session_id: str, history: List[Dict], rewritten_query: str
) -> Dict:
    """
    检查对齐结果，更新 State
    """
...
def step_7_write_history(
    state: Dict, session_id: str, history: List[Dict], rewritten_query: str, message_id: str
) -> Dict:
    """
    写入最终历史记录
    """
...
def node_item_name_confirm(state: QueryGraphState) -> QueryGraphState:
    ...
    with start_span("retrieval.rewrite", attrs={"original_len": len(original_query)}) as rewrite_span:
        extract_res = step_3_extract_info(original_query, history)
        rewritten = extract_res.get("rewritten_query", original_query)
        rewrite_span.set_attribute("rewritten_len", len(rewritten))
