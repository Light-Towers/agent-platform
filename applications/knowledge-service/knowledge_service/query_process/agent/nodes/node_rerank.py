...
from knowledge_service.conf.rerank_config import rerank_cfg
from knowledge_service.core.logger import logger
from knowledge_service.core.tracing import traced_span
from knowledge_service.lm.reranker_utils import get_reranker_model
from knowledge_service.utils.rerank_concurrency import call_under_semaphore, make_rerank_semaphore
...
# 并发推理会放大内存 / 显存占用；超出闸门的调用排队而非丢弃。
_RERANK_SEMAPHORE = make_rerank_semaphore(rerank_cfg.max_concurrency)

# -----------------------------
# Rerank / TopK 配置（M3，方案 §7.3）
# 阈值已外置到 knowledge_service/conf/rerank.yaml 的 dynamic_topk 段：
#   gap_ratio / gap_abs（断崖阈值）、min_k / max_k（上下限）、enabled（开关）。
# 默认值与原硬编码完全一致（gap_ratio=0.25 / gap_abs=0.5 / min=1 / max=10）。
# -----------------------------
...
def step_2_rerank_docs(state, doc_items):
    ...
    try:
        ...
        # M6：并发闸门（threading.BoundedSemaphore(max_concurrency)），限制同时进行模型
        # 推理的调用数，防止多请求打爆显存 / 内存；超出闸门的调用排队而非丢弃（§10.4）。
        scores = call_under_semaphore(_RERANK_SEMAPHORE, reranker.compute_score, sentence_pairs)
...
def step_3_topk(scored_docs):
    """
    阶段三：动态 TopK（最多 max_k）
    基于 scored_docs（已按 score 降序排序）进行智能截断，
    核心逻辑：结合固定上下限+断崖阈值判断，避免机械取前N条，保留语义相关的连续文档集合。
    阈值来自 knowledge_service/conf/rerank.yaml 的 dynamic_topk 段（M3）；enabled=false 时退化为固定取满 max_k。
    :param scored_docs: 列表，元素为带score的文档字典，已按score降序排列，格式如[{"doc": 文档对象, "score": 相关性分数}, ...]
    :return: 列表，动态截断后的TopK文档列表，数量≤max_k
    """
    topk_cfg = rerank_cfg.dynamic_topk
    # 硬上限：最多取 max_k 条，取配置与实际文档数的较小值（避免索引越界）
    max_topk = min(topk_cfg.max_k, len(scored_docs))
    min_topk = topk_cfg.min_k  # 硬下限：至少保留的文档数量
    gap_ratio = topk_cfg.gap_ratio  # 相对断崖阈值：分数下降的相对比例阈值
    gap_abs = topk_cfg.gap_abs  # 绝对断崖阈值：分数下降的绝对差值阈值

    # M3：dynamic_topk.enabled=false 时关闭断崖截断，直接取满 max_k（固定 TopK 模式）
    if not topk_cfg.enabled:
        logger.info(f"Step 3: dynamic_topk 已禁用，固定取前 {max_topk} 条")
        return scored_docs[:max_topk]

    # 1) 断崖截断核心逻辑：从min_topk之后开始检测分数断崖，出现则提前截断
    topk = max_topk  # 默认值：无断崖时取满硬上限（最多 max_k 条）
...
def _rerank_span_attrs(*args, result=None, **kwargs):
    """ranking.rerank span 动态属性（M4，方案 §8.2 表：in_n / out_n / dynamic_topk / fallback_used 等）。"""
    state = args[0] if args and isinstance(args[0], dict) else {}
    in_n = 0
    if isinstance(state, dict):
        in_n = len(state.get("rrf_chunks") or []) + len(state.get("web_search_docs") or [])
    out_docs = (result or {}).get("reranked_docs") or []
    # fallback 探测（启发式）：重排异常时节点返回 score=0.0 的原序文档（既有降级行为）。
    # 真实重排分数几乎不可能全为 0.0，因此"输出非空且全部 score==0"可作为降级标记。
    fallback_used = bool(out_docs) and all(float(d.get("score") or 0.0) == 0.0 for d in out_docs)
    topk_cfg = rerank_cfg.dynamic_topk
    return {
        "in_n": in_n,
        "out_n": len(out_docs),
        "dynamic_topk": bool(topk_cfg.enabled),
        "gap_ratio": topk_cfg.gap_ratio,
        "gap_abs": topk_cfg.gap_abs,
        "min_k": topk_cfg.min_k,
        "max_k": topk_cfg.max_k,
        "fallback_used": fallback_used,
    }


@traced_span("ranking.rerank", attributes_fn=_rerank_span_attrs)
