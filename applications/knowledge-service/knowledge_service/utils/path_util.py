# knowledge_service/utils/path_util.py
"""
项目路径工具。

统一从 `knowledge_service.core.config` 读取 `PROJECT_ROOT`，不再各自加载 .env / 递归查找根目录。
...
from knowledge_service.core.config import PROJECT_ROOT
...
def get_project_root(identifier: str = ".env") -> Path:
    ...
    直接返回全局统一的 PROJECT_ROOT（来自 `knowledge_service.core.config`，由应用启动时统一加载的环境变量保证生效）。
