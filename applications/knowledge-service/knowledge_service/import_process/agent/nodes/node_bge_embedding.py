...
from knowledge_service.lm.embedding_utils import generate_embeddings, get_bge_m3_ef
...
# ==========================================
def _bge_embedding_span_attrs(*args, result=None, **kwargs):
    """ingest.embedding span 动态属性（M4 可选项：导入管线埋点）。"""
    state = result or {}
    return {"chunks_n": len(state.get("chunks") or []) if isinstance(state, dict) else 0}


@traced_span("ingest.embedding", attributes_fn=_bge_embedding_span_attrs)
