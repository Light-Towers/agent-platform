...
from knowledge_service.conf.mineru_config import mineru_config
from knowledge_service.core.logger import logger  # 统一日志工具
from knowledge_service.core.tracing import traced_span

# 项目内部库
from knowledge_service.import_process.agent.state import ImportGraphState, create_default_state
from knowledge_service.utils.format_utils import format_state
...
def step_2_upload_and_poll(pdf_path_obj: Path, output_dir_obj: Path):
    ...
    request_headers = {"Content-Type": "application/json", "Authorization": f"Bearer {MINERU_API_TOKEN}"}
...
def _pdf_to_md_span_attrs(*args, result=None, **kwargs):
    """ingest.pdf_to_md span 动态属性（M4 可选项：导入管线埋点）。"""
    state = result or {}
    return {"md_len": len(state.get("md_content") or "") if isinstance(state, dict) else 0}


@traced_span("ingest.pdf_to_md", attributes_fn=_pdf_to_md_span_attrs)
...
if __name__ == "__main__":
    ...
    test_state = create_default_state(
        task_id="test_pdf2md_task_001", pdf_path=test_pdf_path, local_dir=os.path.join(PROJECT_ROOT, "output")
    )
