...
from knowledge_service.utils.index_registry import register_index
from knowledge_service.utils.task_utils import add_running_task
...
# ==========================================
def _import_milvus_span_attrs(*args, result=None, **kwargs):
    """ingest.import_milvus span 动态属性（M4 可选项：导入管线埋点）。

    chunks_n 取入库后 state.chunks 数量（含 Milvus 回填的 chunk_id）。
    """
    state = result or {}
    return {"chunks_n": len(state.get("chunks") or []) if isinstance(state, dict) else 0}


@traced_span("ingest.import_milvus", attributes_fn=_import_milvus_span_attrs)
...
def create_collection(client, collection_name: str, vector_dimension: int):
    ...
    schema.add_field(
        field_name="chunk_version", datatype=DataType.VARCHAR, max_length=64
    )  # 切分策略版本（切分逻辑变更即升版）
    schema.add_field(field_name="created_at", datatype=DataType.INT64)  # 入库时间戳（epoch 秒，索引时效排查）
    schema.add_field(field_name="source_doc", datatype=DataType.VARCHAR, max_length=65535)  # 源文档名（badcase 溯源）
...
def _enrich_chunk_metadata(item: Dict[str, Any], state: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    仅对缺失字段做缺省填充（setdefault），不覆盖上游已显式写入的值：
        - embedding_model：向量产出模型标识（读统一配置）
...
def step_4_insert_data(client, chunks_json_data: List[Dict[str, Any]], state: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    ...
    核心逻辑：
        ...
        2. 补充版本化元数据字段（M2）：embedding_model / chunk_version / created_at / source_doc
...
def step_5_register_index(chunks_json_data: List[Dict[str, Any]]) -> None:
    """
    步骤4.5：Milvus 入库成功后向索引 registry 登记一条构建记录（M2，方案 §5.4）。

    登记内容：集合名 / 构建时间 / embedding_model / chunk_version / chunk_params /
    doc_count（去重商品数）/ chunk_count（切片数）。eval 内全部指标（recall@5 / mrr /
    ndcg@10 / run_id）由 run_eval.py 评测后回填，此处一律不预填数字。

    设计说明：registry 是辅助治理能力，登记失败仅记录错误日志、不阻断主流程——
    Milvus 数据本身已写入成功，不应因 registry 文件写入失败而误报整个导入失败。
    """
    item_names = {
        str(c.get("item_name", "")).strip() for c in chunks_json_data or [] if str(c.get("item_name", "")).strip()
    }
    try:
        register_index(
            collection_name=CHUNKS_COLLECTION_NAME,
            embedding_model=milvus_config.embedding_model,
            chunk_version=milvus_config.chunk_version,
            chunk_params={"max_len": None, "merge_short": True},
            doc_count=len(item_names),
            chunk_count=len(chunks_json_data or []),
        )
        logger.info(
...
if __name__ == "__main__":
    ...
    # 环境变量已由 knowledge_service.core.config 在导入时统一加载（全项目仅此一次）
