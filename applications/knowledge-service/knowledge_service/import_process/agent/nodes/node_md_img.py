...
from knowledge_service.conf.lm_config import lm_config

# 项目配置
from knowledge_service.conf.minio_config import minio_config
...
from knowledge_service.import_process.agent.state import ImportGraphState

# LLM客户端工具类（核心复用，替换原生OpenAI调用）
from knowledge_service.lm.lm_utils import get_llm_client

# api访问限速工具
from knowledge_service.utils.rate_limit_utils import apply_api_rate_limit
...
def step_3_generate_summaries(
    doc_stem: str, targets: List[Tuple[str, str, Tuple[str, str]]], requests_per_minute: int = 9
) -> Dict[str, str]:
    """
...
def upload_images_batch(
    minio_client: Minio, upload_dir: str, targets: List[Tuple[str, str, Tuple[str, str]]]
) -> Dict[str, str]:
    """
...
def node_md_img(state: ImportGraphState) -> ImportGraphState:
    ...
    # 步骤3：生成图片摘要（可选增强，缺省关闭 —— 无 VL 能力时跳过，图片标题用默认值）
    if settings.img_summary_enabled:
        summaries = step_3_generate_summaries(path_obj.stem, targets)
    else:
        logger.info("IMG_SUMMARY_ENABLED=false，跳过图片摘要（VL 模型非必需，标题用默认值）")
        summaries = {img_file: "图片描述" for img_file, _, _ in targets}
