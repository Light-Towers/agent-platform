...
from knowledge_service.utils.normalize_sparse_vector import normalize_sparse_vector
...
def step_2_build_context(
    chunks: List[Dict], k: int = DEFAULT_ITEM_NAME_CHUNK_K, max_chars: int = CONTEXT_TOTAL_MAX_CHARS
) -> str:
    """
...
def step_3_call_llm(file_title: str, context: str) -> str:
    ...
    try:
        ...
        # 规范化商品名：剥离品牌前缀/内部料号，保证同型号跨文档识别结果一致（与检索侧共用口径）
