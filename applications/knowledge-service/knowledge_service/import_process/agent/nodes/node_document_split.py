...
from knowledge_service.core.logger import logger  # 项目统一日志工具，核心替换print
...
def _document_split_span_attrs(*args, result=None, **kwargs):
    """ingest.document_split span 动态属性（M4 可选项：导入管线埋点）。"""
    state = result or {}
    return {"chunks_n": len(state.get("chunks") or []) if isinstance(state, dict) else 0}


@traced_span("ingest.document_split", attributes_fn=_document_split_span_attrs)
...
if __name__ == "__main__":
    ...
    from knowledge_service.import_process.agent.nodes.node_md_img import node_md_img
