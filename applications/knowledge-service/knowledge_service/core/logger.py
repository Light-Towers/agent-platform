"""
项目日志工具类
基于loguru实现，支持统一配置控制台/文件双输出，自动生成logs/app_年月日.log
特性：
1. 配置驱动：通过 knowledge_service.core.config 开关输出、修改日志级别
...
# 统一配置：项目根目录与日志开关均来自 knowledge_service.core.config（全项目唯一配置入口）
from knowledge_service.core.config import PROJECT_ROOT, settings
...
LOG_FILE_ENABLE = settings.log_file_enable.lower() == "true"
...
def init_logger():
    ...
    if LOG_CONSOLE_ENABLE:
        logger.add(sink=sys.stdout, level=LOG_CONSOLE_LEVEL, format=LOG_FORMAT, colorize=True, enqueue=True)
...
def fix_log_position(record):
    ...
    for frame in inspect.stack():
        ...
        if ("_logger.py" in frame.filename or frame.function == "_log") or "logger.py" in frame.filename:
            continue
        # 更新日志字段为业务代码实际位置
        record.update(name=frame.filename.split("/")[-1].split("\\")[-1], function=frame.function, line=frame.lineno)
        break
