...
# ===================== 唯一一次加载 .env =====================
# 仅在此处调用，确保全项目只有一次 load_dotenv；其余模块直接 from knowledge_service.core.config import settings
load_dotenv()
...
class Settings:
    ...
    # api 模式下稀疏向量由 knowledge_service/lm/sparse_vectorizer.py 本地生成（B 路线），保住双路混合检索。
