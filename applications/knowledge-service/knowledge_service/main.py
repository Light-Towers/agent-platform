...
统一在 knowledge_service.core.config 中完成一次 .env 加载，全项目共享配置单例。
"""
...
# 全局应用实例：供 uvicorn 以 `knowledge_service.main:app` 直接加载
app = create_app()


def run() -> None:
    """命令行入口（pyproject scripts：knowledge-service）：以 uvicorn 启动服务。"""
    import uvicorn
