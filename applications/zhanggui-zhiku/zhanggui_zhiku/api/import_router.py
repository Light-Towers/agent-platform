...
跨域（CORS）统一在 zhanggui_zhiku.main.create_app() 中配置，本路由不再单独处理。
"""
...
async def get_import_page():
    ...
    html_abs_path = PROJECT_ROOT / "zhanggui_zhiku/import_process/page/import.html"
...
async def upload_files(background_tasks: BackgroundTasks, files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        minio_pdf_base_dir = settings.minio_pdf_dir  # 缺省值：pdf_files（见 zhanggui_zhiku.core.config）
        ...
        try:
            ...
            # 从统一配置获取MinIO的桶名配置
            minio_bucket_name = settings.minio_bucket_name  # 缺省值：kb-import-bucket（见 zhanggui_zhiku.core.config）
