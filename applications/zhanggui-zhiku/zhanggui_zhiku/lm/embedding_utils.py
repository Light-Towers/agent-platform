...
def _get_api_embedding_client():
    ...
    if _api_embedding_client is None:
        ...
        from zhanggui_zhiku.lm.siliconflow_client import SiliconFlowEmbeddingClient
...
def _generate_embeddings_api(texts):
    ...
    → 业务节点零改动。
    """
    from zhanggui_zhiku.lm.sparse_vectorizer import build_sparse_vectors
...
def generate_embeddings(texts):
    ...
    稀疏由 zhanggui_zhiku/lm/sparse_vectorizer.py 本地生成（B 路线）；返回结构与 local 完全一致。
